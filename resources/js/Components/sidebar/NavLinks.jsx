...
export default function NavLinks({ isSidebarOpen }) {
    ...
    // emp_id/emp_name/emp_dept/emp_station/emp_prodline/etc.). These hide
    // department-specific links for everyone outside that department --
    // purely a UX nicety, every route is already guarded server-side
    // (PeValidationController::assertIsPe(), QdnWorkflowController's
    // assertIsPe()/assertIsQa()/assertOwningDepartment(), all of which now
    // use the same DEPARTMENT_PE/DEPARTMENT_QA constants as below).
    //
    // "PE" turned out to be shorthand, not the real stored value -- a
    // logged-in PE employee's actual emp_dept is "Process Engineering".
    // Matching case-insensitively + trimmed here too, same as
    // CurrentEmployee::isInDepartment() on the backend, so this doesn't
    // silently break again over a casing/whitespace difference.
    ...
    const isQa = ["quality assurance", "quality management system"].includes(
        normalizedDept,
    );
